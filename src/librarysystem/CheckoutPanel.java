...
public class CheckoutPanel extends JPanel {
	...
	private final String[] headerNames = { "ISBN", "Title", "Copy number", "Library Member", "Checkout Date",
			"Due Date" };
	...
	private void addCheckoutPanel() {
		...
		checkoutButton.addActionListener(e -> {
			...
			try {
				...
				JOptionPane.showMessageDialog(this, "Checkout successfull!", "Success",
	...
	private void addEntryToDataSource(DefaultTableModel dataSource, BookCopy bookCopy, CheckoutRecordEntry entry) {
		if (entry != null && entry.getCheckoutRecord() != null) {
			dataSource.addRow(
					new Object[] { bookCopy.getBook().getIsbn(), bookCopy.getBook().getTitle(), bookCopy.getCopyNum(),
							entry.getCheckoutRecord() != null
									? entry.getCheckoutRecord().getLibraryMember().getFullNameWithId()
									: null,
							entry.getCheckoutDate(), entry.getDueDate() });
		} else {
			dataSource.addRow(new Object[] { bookCopy.getBook().getIsbn(), bookCopy.getBook().getTitle(),
					bookCopy.getCopyNum(), null, null, null });
		}
