...
public class AddNewMemberPanel extends JPanel {
	...
	private JTextField memberIdField, firstNameField, lastNameField, streetField, cityField, stateField, zipField,
			telephoneField;
	...
	public AddNewMemberPanel() {
		...
		/*
		 * JPanel titledPanel = new JPanel(new BorderLayout()); JLabel jLabel = new
		 * JLabel("Create new member"); jLabel.setFont(new Font("Arial", Font.BOLD,22));
		 * jLabel.setOpaque(false); titledPanel.add(jLabel,BorderLayout.NORTH);
		 */
		...
		for (LibraryMember member : mems) {
			...
			Object[] rowDatas = { member.getMemberId(), member.getFirstName(), member.getLastName(),
					member.getAddress().getStreet(), member.getAddress().getCity(), member.getAddress().getState(),
					member.getAddress().getZip(), member.getTelephone() };
	...
	private boolean validateAndSave() {
		...
		Object[] rowData = { memberId, firstName, lastName, street, city, state, zip, telephone };
		modelRight.addRow(rowData);
